...
**Project name (`-p`/`--project-name`):** compose derives a project name from the working directory when none is given, which is *not* the project anything deployed with `docker compose -p` or `COMPOSE_PROJECT_NAME` runs under — several such projects can share one directory. cdeploy resolves the project itself: it looks the directory up in `docker compose ls` and, when exactly one project lives there, addresses that project by its real name and loads its own compose files. Pass `-p` to pick one when several share a directory. `-p` selects an **existing** project — a name docker does not report is refused, because running the pipeline under that label would recreate it from whatever compose file the directory happens to hold. CLI-only (the TUI picker already carries each row's real name), and `list` requires `-C` alongside it, because `list` without `-C` discovers every project on the host.
...
- Stored on the docker host at `~/.cdeploy/state/<hash>.json` (`sha256(project dir + NUL + project name)`, first 12 hex chars), so a CI deploy and a laptop rollback share one authoritative history. The project name is part of the hash because two `-p` projects can share one directory and would otherwise share one snapshot file — a rollback of one restoring the other's images. The name is resolved from `docker compose ls`, not from how the command was typed, so the TUI, a `-p` run and a bare `-C` run of the same project all key one file. A project docker does not report keys the directory alone, exactly as before; a named project also reads the directory-only file when it has no file of its own yet, so upgrading orphans nothing.
