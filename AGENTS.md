...
No linter, formatter, or CI config — use standard `go fmt` and `go vet`. No Makefile or task runner.
...
**Docker Compose**: All docker interaction goes through `compose.Compose.command()` (local) or `compose.RemoteCompose.remoteCommand()` (SSH) which builds `exec.CommandContext("docker", "compose", ...)` with `CURRENT_UID` env var (format `uid:gid` locally, `$(id -u):$(id -g)` on remote). Uses v2 plugin syntax only (`docker compose`, not `docker-compose`). Exact subcommands: `stop`, `rm -f`, `pull`, `up --no-start`, `start`, `config --services`, `ps -a --format json`, `logs`. Project discovery uses `docker compose ls -a --format json`. Empty container slice means operate on all services. `parseContainerStatus()` handles both JSON array output (Docker Compose v2.21+) and NDJSON (older versions). CLI subcommands reject `-a` combined with explicit container names.
...
`scripts/` contains the original bash scripts this tool replaces — they use v1 `docker-compose` syntax. Reference only.
