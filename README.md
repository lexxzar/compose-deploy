...
- **IMAGE** — the configured image reference, the image id docker resolved it to, and the command and entrypoint. When the [update check](#update-available-indicators---updates) says a newer image is waiting, four more rows appear: `built` (when the running image was built), `update` (the verdict and how old the check behind it is), `update id` (the image id the new image will have once pulled — directly comparable with the `image id` row above) and `update built`. Each row is omitted on its own when the value is unknown; images built reproducibly report `1970-01-01`, which cdeploy treats as no answer rather than a date
