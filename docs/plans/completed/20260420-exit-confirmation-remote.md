...
- [x] Add `quitting bool` field to `Model` struct (near the `confirming` field at line ~181)
- [x] Add `tryQuit()` method: check `disconnectFunc != nil` → set `quitting = true` and return, else return `tea.Quit`
- [x] Add `quitting` intercept at the top of `handleKey()` (before `switch m.screen`): `y` → `tea.Quit`, `n`/`esc` → `quitting = false`, swallow other keys
- [x] Replace `return m, tea.Quit` with `return m.tryQuit()` in 8 locations (all screens except `screenSelectServer`): `screenSelectProject`, `screenSelectContainers` normal, `screenSelectContainers` confirming, `screenProgress`, `screenLogs`, `screenConfig`, `screenSettingsList`, `screenSettingsForm`
- [x] Reset `quitting = false` in `esc` back-navigation handlers that clear `disconnectFunc` (going back to server select)
- [x] Write tests: `q` on remote connection → `quitting == true`, screen unchanged
- [x] Write tests: `q` on local (no disconnectFunc) → `tea.Quit` returned
- [x] Write tests: `quitting` + `y` → `tea.Quit`, `n` → cancel, `esc` → cancel, other key → swallowed
- [x] Write test: `q` on server select screen → always `tea.Quit` directly
- [x] Run tests: `go test ./internal/tui/ -count=1`
...
- [x] Add `viewQuitConfirm()` method that renders a standalone confirmation screen: title + `"Disconnect from {serverName}? (y/n)"`
- [x] In `View()`, add `if m.quitting` check at the top that returns `viewQuitConfirm()`
- [x] Write test: `View()` output contains "Disconnect from" when `quitting == true` and `serverName` is set
- [x] Run tests: `go test ./internal/tui/ -count=1`
...
- [x] Update CLAUDE.md with quit confirmation pattern description (in the TUI state machine section)
