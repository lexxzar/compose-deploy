...
- [x] update `README.md` — new `## Container Inspect Screen` section (the five summary sections, the `r` toggle, the scroll/leave keys, the Uptime-replica rule and the named no-container error), the screen count raised to seven with an `Inspect` entry in the screen list, `i` added to the service-screen key list and to the unmanaged screen's works-as-usual row, and a blockquote stating plainly that env values — `POSTGRES_PASSWORD`, `DATABASE_URL`, API tokens — are shown verbatim in BOTH modes and leak on a screen-share
- [x] update `CLAUDE.md` with an "Inspect screen" paragraph (inserted between **Config screen** and **Exec into container**), covering all five required points plus the parse-layer location, the pointer-`Health` distinction, the three buffer helpers, the `GotoTop` rationale and the two-round-trip cost; `Inspector` also named in **Package Coupling** (same shape as `ReadOnlyComposer`, reach inverted — all three composers implement it) and in the two **Testing Approach** bullets; the **TUI state machine** paragraph now reads seven screens and names the `i` key beside `c`
- [x] update `CLAUDE.md`'s documented truncation table with the re-measured heights from Task 10 — the table now reads >= 24 / 23 / 22 / 21 / 20 / 19, the stacked-row math reads 30 → 25 with 19 action rows fitting the 20 a 24-line pane keeps, the read-only 16-row never-truncates note is added, the nowhere-else list gained `i inspect`, the container key table is 19 rows, `covers all 8 screens` → 9, `not a ninth screen constant` → `not a screen constant of its own`, and `TestViewHelp_InspectSurvivesTheFirstTruncation` is named beside the other pins
- [x] ➕ record the repo residual in `CLAUDE.md`'s `?` overlay paragraph: the bound USED to read `int(screenSettingsForm) + 1`, so `screenInspect` could be added with the suite green while the six `allScreens`-iterating tests silently skipped it. Task 7 fixed the bound to `int(screenInspect) + 1` and reworded the comment, so what is recorded is the history, the six tests it silently skipped, and the rule that the bound must move to the new last constant with every screen added — the tax is developer discipline, not a failing test
- [x] record the residual that `i` is deliberately absent from the container footer — in `CLAUDE.md`'s container-footer paragraph, beside the 19-row table sentence: a seventh token would re-open the 80-column guarantee and the width 40-180 `TestContainerFooterReservation` sweep, so `i` lives in the `?` overlay only, exactly like `c`, `x`, `U` and `R`, and the accepted cost is that inspect is discoverable through `?` alone
- [x] move this plan to `docs/plans/completed/` (deferred to the orchestrator's terminal move — review phases still read the plan in place)
