...
2. **Services** — one screen with two modes. The **host view** shows every Docker Compose project on the host as a foldable group, plus an `(unmanaged)` group for containers no project owns — see [Unmanaged containers](#unmanaged-containers-read-only). `space` on a group header folds it, `enter` drills into that project, and a selection may span projects: the operation then runs one pipeline per project, in screen order, and stops at the first failure. Drilling in gives the **single-project view**, which is the same screen over one project and also lists services that have never been created (the host view can only show containers that exist). `esc` drills back out to the host view, and again to the server list. **With nothing selected, `d`/`r`/`s` in the host view act on the WHOLE group under the cursor** — cursor on a `web` row or on the `web` header, press `d`, and the prompt reads `Deploy web (all)?`, which is every service in that project including ones that have never been created. Read the prompt before confirming. The single-project view keeps the older, stricter rule: an action key with nothing selected warns `No service is selected` and does nothing. A group cdeploy found only through running containers — one `docker compose ls` did not report, so no project directory is known for it — refuses operations, drill-in and rollback with a warning rather than guessing a directory. In both modes: pick services (`space` toggles one, `a` toggles all — in the host view, all the services in the groups that are currently expanded) and choose an action (`r` restart, `d` deploy, `s` stop, `R` rollback, `l` logs, `i` inspect, `c` config, `x` exec, `U` re-check updates); press `/` to search-and-jump to a service by name substring and `n`/`N` to cycle through matches (search moves the cursor and highlights matches without filtering the list or touching your selection); the arrows or `j`/`k` move one row and `pgup`/`pgdown` move one screenful at a time; also shows CPU% and Mem (used/limit) columns for running services, refreshed on screen entry and after every operation. **In the host view, `a` leaves folded groups alone.** A group you folded keeps whatever was selected in it, that selection still runs when you press an action key, and the group's header shows a `[x] n/m` count so a hidden selection stays visible. If every group is folded there is nothing for `a` to select, and it says so rather than ticking rows you cannot see. The `(n/m selected)` counter in the title covers the whole host, folded groups included, so it can report more selected services than the screen is showing. Services whose registry image is newer than the local copy get a yellow `⇧` marker next to the service name; the indicator is cached for 10 minutes and `U` forces a refresh. `R` reads the host-side deploy snapshot and, when one exists, asks to confirm a digest-pinned rollback of the selected services (the prompt shows how long ago the snapshot was recorded). **`R` covers ONE project at a time**: a rollback restores digests from that project's own snapshot file, so a selection that spans projects is refused with `Rollback covers one project at a time` — select within a single group and press `R` again. `d`, `r` and `s` are the keys that may span projects; `R` is not. The footer shows only the most-used keys; press `?` for the full key list for the current screen. Rows in the `(unmanaged)` group are read-only — see [Unmanaged containers](#unmanaged-containers-read-only).
3. **Progress** — watch step-by-step execution with status indicators. After a deploy, restart, or rollback the screen enters a **health-wait** sub-state: it polls each targeted service and shows a live per-service verdict (`♥` healthy, `●` running with no healthcheck, `✗` failed, `~` pending) with a countdown to the timeout. In a single-project run, press `esc` to skip the wait (the operation stays "done"). In a multi-project run the gate binds two keys with opposite effects: `enter` skips the wait and starts the next project, `esc` stops the sequence and marks the projects left `(skipped)`. The footer names both. A failed deploy wait shows the hint `press R on the services screen to roll back`. An operation that spans several projects shows every project's steps up front, prefixed with the project name, and each project's health wait must pass before the next project starts; a failure marks the remaining projects `(skipped)`.
...
`?` is typeable inside the container search bar, the log filter and search bars, and settings-form text inputs, so a regular expression such as `(?:web|db)` reaches the input instead of opening the key list. `?` does nothing while a confirmation prompt is armed. On a very narrow or very short terminal the key list is trimmed to fit and ends with a `▼ N more` marker — resize the window to see the rest. The host view needs more room than the other screens: in an 80x24 window its key list is trimmed too, and `x exec` and `U check updates` are the entries that go — widen the window past 86 columns, or make it at least 26 lines tall, to see them.
...
**These rows are read-only.** A container with no compose file cannot be deployed, rolled back, or shown a config. On the host view the unmanaged rows carry no selection checkbox and the write keys refuse while the cursor sits on one. Press `enter` on any `(unmanaged)` row to drill in, and the whole screen switches to its read-only variant, where those keys are absent from the footer and from the `?` key list as well:

| Key | On an unmanaged row |
|-----|---------------------|
| `l` `i` `x` `U` | Work as usual — logs, inspect, exec, force an update check |
| `/` `n` `N` `esc` `q` `?` arrows `pgup` `pgdown` | Work as usual — search, navigate, page, back, key list |
| `d` `r` `s` `R` `c` `space` `a` | Refuse — deploy, restart, stop, rollback, config, and multi-select need a compose project |

**"Unadvertised" means the drilled read-only view.** In that view — and on a host whose ONLY group is `(unmanaged)`, where the whole screen is read-only anyway — the refusing keys are dropped from the footer and from `?`, so nothing offers you a key that does nothing. On a mixed host view they stay advertised, because they work on every other group; they simply refuse while the cursor is parked on an unmanaged row. `enter` is the exception: it still drills in on an unmanaged-only host, so `?` names it there, while the footer keeps its four tokens (a lone group draws no header, so the drilled screen shows the same rows). The host-view footer also differs from the single-project one: `space` reads `space fold/select` (a header folds, a service row selects), `enter drill in` takes the slot `l logs` holds on the single-project screen, and `l` moves to the `?` key list.
...
**Detail rows on the inspect screen (TUI only).** When a service's verdict is `true`, the TUI fetches a little more about that image so the [inspect screen](#container-inspect-screen)'s IMAGE section can say *which* image is waiting and *when it* was built: one local `docker image inspect` for the platform to match against, then `docker buildx imagetools inspect` up to three more times — `--format '{{json .Manifest}}'` to pick the host's platform out of a multi-arch index, `--raw` for the config digest (which is the image ID the image gets once pulled), and `--format '{{json .Image}}'` for its build time. That is up to three extra registry round-trips per *updated* image, on top of the one the check itself makes. It is two rather than three whenever the reference resolves to a single manifest instead of a multi-arch index: the first call has then already returned the manifest itself, so its config digest is reused and the `--raw` call is skipped. It rides the same cache entry, the same 10-minute TTL, the same `U` key and the same post-deploy invalidation as the glyph, and it runs only for services that actually came back `true` — usually a small minority. `cdeploy list --updates` does not pay this cost; the CLI has no detail view to render it in. A failure here is silent: the `⇧` glyph is unaffected and the extra rows are simply omitted, and any value the parsers cannot fully trust is omitted rather than guessed. The platform match includes the CPU variant: the index usually carries two entries for `linux/arm` (`v5`+`v7`, or `v6`+`v7`), and the local probe reports the variant the running image was built for, so the matching entry is used. An image config that names no variant is resolved the way Docker itself resolves it — `arm` means ARMv7, and `arm64` and `arm64/v8` are the same platform — so an unqualified `linux/arm` entry is never handed to an ARMv5 or ARMv6 host. A host whose variant is not in the index at all — an ARMv6 host offered only `v5` and `v7` — gets no rows rather than another image's. These rows also need the buildx plugin (Docker v23+) — the `docker manifest inspect` fallback the check itself uses does not apply here, so on an older host the glyph still works and the rows never appear. The verdicts are delivered as soon as the check returns, so the `⇧` glyphs paint at their usual speed and these rows arrive a moment later. If you hit `429 Too Many Requests` from Docker Hub, `docker login` on the docker host raises the anonymous quota; a detail-only failure is cached alongside the successful verdicts for the full 10 minutes, so press `U` to retry it sooner.
...
-p, --project-name string  Docker compose project name (default: derived from the project directory)
    --log-dir string       Log directory (default ~/.cdeploy/logs/)
```

**Project name (`-p`/`--project-name`):** compose derives a project name from the working directory when none is given, which is *not* the project anything deployed with `docker compose -p` or `COMPOSE_PROJECT_NAME` runs under — several such projects can share one directory. Pass `-p` to address one of them. cdeploy passes it straight through as compose's own `-p`, so the CLI behaves exactly like `docker compose -p <name>` run in the project directory: `-p` may name a project that does not exist yet (a first deploy creates it), and it never overrides `-C`. CLI-only (the TUI picker already carries each row's real name), and `list` requires `-C` alongside it, because `list` without `-C` discovers every project on the host.

Because cdeploy has no `-f` flag, a `-p` run relies on compose's file auto-discovery in `-C`. A project built from a non-default file set (`docker compose -f prod.yml -p prod up`) in a directory that also holds a `docker-compose.yml` would be recreated from the wrong file, so drive that project from the TUI instead — its picker rows carry each project's own file set, straight from docker.
...
- Stored on the docker host at `~/.cdeploy/state/<hash>.json` (`sha256(project dir + NUL + project name)`, first 12 hex chars), so a CI deploy and a laptop rollback share one authoritative history. The project name is part of the hash because two `-p` projects can share one directory and would otherwise share one snapshot file — a rollback of one restoring the other's images. The name in the hash is **empty for the project the directory itself names**, which is the project a command with no `-p` addresses: the TUI's fast track, a drill-in on that project's row and a bare `cdeploy rollback -C <dir>` therefore all key the one file every earlier cdeploy wrote, and upgrading orphans nothing. Nothing looks the name up on the host — a snapshot must stay findable after a failed deploy has already removed every container, which is exactly when `docker compose ls` stops reporting the project.
...
- **IMAGE** — the configured image reference, the image id docker resolved it to, when that image was built, and the command and entrypoint. The `built` row does not depend on the update check: the inspect screen asks the local docker daemon about the image the container actually resolved to — not the tag the compose file names, which a `docker pull` can move off the running container — so it appears for every container, on a cold update cache, in the grouped host view and on the read-only unmanaged screen alike. One extra row, `update`, appears for any service the [update check](#update-available-indicators---updates) reached — `available` or `up to date`, with how old the check behind it is. The other two need an actual update, because they are fetched only for a `true` verdict: `update id` (the image id the new image will have once pulled — directly comparable with the `image id` row above) and `update built`. A value the parsers cannot trust omits its own row — images built reproducibly report `1970-01-01`, which cdeploy treats as no answer rather than a date — and a *failed* registry call omits `update id` and `update built` together, since the fetch stops at the first failure. `update` survives that, because it needs no fetch beyond the check itself, and so does `built`, which asks no registry at all; only a failed local probe drops it
