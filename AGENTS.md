...
No linter or formatter config — use standard `go fmt` and `go vet`. No Makefile or task runner. CI runs `go test ./... -covermode=atomic -coverprofile=coverage.out` via GitHub Actions (`.github/workflows/coveralls.yml`).
