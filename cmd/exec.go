...
func runExec(ctx context.Context, service string, command []string) error {
	...
	switch {
	case sshTarget != "":
		rc, cleanup, err := resolveSSHRemote(ctx, sshTarget, projectDir, projectName, identityFile, execNewRemote)
		if err != nil {
			return err
		}
		defer cleanup()
		c = rc
	case serverName != "":
		rc, cleanup, err := resolveServerRemote(ctx, serverName, projectDir, projectName, execNewRemote)
		if err != nil {
			return err
		}
		defer cleanup()
