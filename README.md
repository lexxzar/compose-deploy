...
**Project name (`-p`/`--project-name`):** compose derives a project name from the working directory when none is given, which is *not* the project anything deployed with `docker compose -p` or `COMPOSE_PROJECT_NAME` runs under — several such projects can share one directory. Pass `-p` to address one of them explicitly; `docker compose ls` reports the names in use. CLI-only (the TUI picker already carries each row's real name), and `list` requires `-C` alongside it, because `list` without `-C` discovers every project on the host.
