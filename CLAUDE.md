...
**Config**: `internal/config` loads `~/.cdeploy/servers.yml` defining remote servers with `name`, `host`, optional `project_dir`, optional `group`, and optional `color` fields. `color` accepts ANSI color names: red, green, yellow, blue, magenta, cyan, white, gray; invalid values are rejected by `Validate()`. Servers with the same `group` are displayed together under a shared header in the TUI. `Load()` returns an empty config (not error) if the file doesn't exist. SSH-specific options (keys, jump hosts, tunnels) belong in `~/.ssh/config`.
